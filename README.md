...
|-----------|---------------|---------|
| API framework | `@nestjs/common` | 10.x |
| Language | TypeScript | 5.x |
| Database | PostgreSQL | 16 |
| ORM | `typeorm` + `@nestjs/typeorm` | 0.3.x |
| Timezone conversion | `luxon` | 3.x |
| Request validation | `class-validator` + `class-transformer` | 0.14.x |
...
Copy `.env.example` to `.env` before starting. All variables are required unless marked optional.

| Variable | Description | Example |
|----------|-------------|---------|
| `NODE_ENV` | Runtime environment (`development`, `test`, `production`) | `development` |
| `APP_PORT` | TCP port the NestJS server listens on | `3000` |
| `DB_HOST` | PostgreSQL hostname (`localhost` when running outside Docker) | `localhost` |
| `DB_PORT` | PostgreSQL port | `5432` |
| `DB_USERNAME` | Database username | `sessiongrid` |
| `DB_PASSWORD` | Database password | `changeme` |
| `DB_NAME` | Database name | `sessiongrid` |
| `JWT_SECRET` | Secret used to sign JWT tokens — must be long and random in production | `replace-with-a-long-random-secret` |
| `JWT_EXPIRES_IN` | How long access tokens remain valid (ms-style string) | `7d` |
...
Five tables. All timestamp columns are `TIMESTAMPTZ` (UTC). Primary keys are UUIDs generated by the database.

### `users`

Stores every registered account. One row per user regardless of role.

| Column | Type | Notes |
|--------|------|-------|
| `id` | uuid PK | |
| `email` | varchar | Unique |
| `password_hash` | varchar | scrypt-derived; salt prepended with `:` separator |
| `first_name`, `last_name` | varchar | |
| `role` | enum | `teacher` or `parent` |
| `timezone` | varchar | IANA timezone string (e.g. `Asia/Kolkata`) |
| `created_at`, `updated_at` | timestamptz | |

A user with role `teacher` owns courses. A user with role `parent` holds bookings.

### `courses`

A subject or class created and owned by one teacher.

| Column | Type | Notes |
|--------|------|-------|
| `id` | uuid PK | |
| `teacher_id` | uuid FK → `users` | Restricted delete |
| `title` | varchar | |
| `description` | text | Nullable |
| `created_at`, `updated_at` | timestamptz | |

Index on `teacher_id`. One course has many offerings.

### `offerings`

A scheduled run of a course — has a capacity and a lifecycle status.

| Column | Type | Notes |
|--------|------|-------|
| `id` | uuid PK | |
| `course_id` | uuid FK → `courses` | Restricted delete |
| `capacity` | integer | Maximum number of bookings (stored; not enforced at booking time) |
| `status` | enum | `draft`, `published`, or `cancelled` |
| `created_at`, `updated_at` | timestamptz | |

Indexes on `course_id` and `status`. One offering has many sessions and many bookings. Only `published` offerings can be booked.

### `sessions`

Individual time slots within an offering. Times are stored as UTC; the service layer converts them to each user's local timezone on read.

| Column | Type | Notes |
|--------|------|-------|
| `id` | uuid PK | |
| `offering_id` | uuid FK → `offerings` | Cascade delete |
| `starts_at` | timestamptz | UTC |
| `ends_at` | timestamptz | UTC |
| `created_at`, `updated_at` | timestamptz | |

Check constraint `CHK_sessions_ends_after_starts` enforces `ends_at > starts_at`. Composite index on `(offering_id, starts_at, ends_at)` accelerates the overlap query inside the booking transaction.

### `bookings`

Records a parent's confirmed booking for a published offering.

| Column | Type | Notes |
|--------|------|-------|
| `id` | uuid PK | |
| `offering_id` | uuid FK → `offerings` | Restricted delete |
| `parent_id` | uuid FK → `users` | Restricted delete |
| `status` | enum | `confirmed` or `cancelled` |
| `created_at`, `updated_at` | timestamptz | |

Unique constraint `UQ_bookings_offering_id_parent_id` on `(offering_id, parent_id)` prevents a parent from holding more than one booking for the same offering. Index on `parent_id` for fast lookup of a parent's bookings. Cancellation deletes the row, so the unique constraint never blocks re-booking.
